...
public class MathFactGame_A extends PApplet {
...
//Is this necessary?
public void selectbuffFile(File selection){
...
class Board {
  ...
  public void drawBoard(ArrayList<BoardObj> objs){
...
class Brick extends BoardObj{
  ...
  Brick(Problem prob, Datum d){
    ...
    myProb = prob;
    datum = d;
    sign = PApplet.parseInt(random(0,2));
    if(sign==0)sign = -1;
    burst = false;
    burstDone = false;
  } 
  ...
  void display(){
    if(!burst){
      xpos = PApplet.parseInt(temp_x+increase_x);
      ypos = PApplet.parseInt(temp_y-increase_y);
      pushStyle();
  ...
  public void update(float inc){
    ...
    increase_x += inc*sign/PApplet.parseFloat(fr);
    increase_y += inc/PApplet.parseFloat(fr);
    //println("inc="+inc+" x_inc="+increase_x+" y_inc="+increase_y);
  } 

  public void addResponse(String resp, int onset){
...
class Datum{
  ...
  public void addResponse(String resp, int respTime){
    if (responses.equals("")){
      responses+=resp;
...
class Data{
  ...
  public Datum addNewDatum(Problem curProblem, int onset, float skillLevel, int round, int probNum, int time_allotted){
    Datum newDatum = new Datum(curProblem, onset, skillLevel, round, probNum, time_allotted);
    dataList.add(newDatum);
    return newDatum;
  }

  public Datum getCurDatum(){
...
class Game{
  ...
  Game(){
    ...
    skillLevel = distanceBubbleTravels/PApplet.parseFloat(time_allotted)*1000.0f;
  ...
  public void storyScreenA(){
    if(ship_x<boardx0+boardWidth-shipWidth){
      //println(ship_x+","+ship_y);
      int increase =  PApplet.parseInt(10*sin(PI/2.0f*PApplet.parseFloat(millis()+1000)/1000.0f))-10;
      ...
      ship_x+=5;
      ship_y+=PApplet.parseInt(5*sin(PI/2.0f*PApplet.parseFloat(millis())/1000.0f));
  ...
  public void sendMouseToListener(int x, int y){
    if(gamestate==0){
  ...
  public void createbuffReader(String pid, String day){
  ...
  public void loadData(String pid, String day){
  ...
  public ClickableObj findMouseSelect(int x, int y){
  ...
  private void printStatus(){
    ...
    println(distanceBubbleTravels/PApplet.parseFloat(time_allotted)*1000.0f);
...
class Round{
  ...
  public ArrayList<Problem> setupProblems(){
  ...
  public Problem addNewProb(String stim, String ans){
...
class TextField extends ClickableObj{
  ...
  public void changeValue(char key){
    if(key==BACKSPACE){ 
      if(value.length()>0){
...
class Title extends BoardObj{
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "MathFactGame_A" };
    if (passedArgs != null) {
